...
func TestLinuxStrip_Init(t *testing.T) {
	type args struct {
		gpio       int
		pixels     int
		brightness int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.s.Init(tt.args.gpio, tt.args.pixels, tt.args.brightness); (err != nil) != tt.wantErr {
				t.Errorf("LinuxStrip.Init() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func TestLinuxStrip_Render(t *testing.T) {
	type args struct {
		strip frame.HexGRB
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := tt.s.Render(tt.args.strip); (err != nil) != tt.wantErr {
				t.Errorf("LinuxStrip.Render() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
